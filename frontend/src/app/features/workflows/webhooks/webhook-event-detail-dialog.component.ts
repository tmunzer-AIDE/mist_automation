...
@Component({
      ...
      @if (loading) {
        ...
        <div class="info-grid">
          <div class="info-item">
            ...
            <span class="info-value">{{ event.received_at | relativeTime }}</span>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            ...
            <span class="info-value">
              <app-status-badge [status]="event.signature_valid ? 'active' : 'failed'"></app-status-badge>
            </span>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            ...
            <span class="info-value">{{ event.matched_workflows.length }} workflow(s), {{ event.executions_triggered.length }} execution(s)</span>
          </div>
        </div>
  ...
  styles: [`
    ...
    .chip-automation { background: #eff6ff; color: #1d4ed8; }
    .chip-backup { background: #ecfdf5; color: #047857; }
    .http-status {
      font-family: monospace;
      font-weight: 600;
      font-size: 13px;
    ...
    .status-client-error { background: #fef2f2; color: #b91c1c; }
    .status-server-error { background: #fef2f2; color: #991b1b; }
    .tab-content {
      padding: 16px 0;
    }
    .json-header {
...
export class WebhookEventDetailDialogComponent implements OnInit {
  ...
  event: WebhookEventDetail | null = null;
  ...
  ngOnInit(): void {
    this.webhookEventService.getEvent(this.data.eventId).subscribe({
      next: (ev) => {
        this.event = ev;
        this.headerEntries = Object.entries(ev.headers || {});
        this.loading = false;
